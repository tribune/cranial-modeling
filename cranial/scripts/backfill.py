...
1.  Dataset(keys_list:(str, tuple, list), **other_params)
    with method get_data() which returns an instance of ReIter subclass with data that is needed to be processed.
...
log = logger.get(name='backfill_job', var='BACKFILL_LOGLEVEL')
