"""
Standard script for running a cranial model transformation on a data-feed from kafka.

There should be a model.py file in which these three objects are defined:
1.  Dataset(keys_list:(str, tuple, list), **other_params)
    with method get_data() which returns an instance of ReIter subclass
2.  Model(**kwargs) with methods
        - itransform(iter_data) which returns a re-iterator with transformed data
        - load(s3_connector), which can be doing nothing. TODO: make load as an option to the script
3.  Consumer() - an object that has method consume(input_iterator) which triggers
    iterations of the input iterator, can be a simple for loop with doing nothing inside.
...
from cranial.listeners.kafka import Kafka
...
def run(opts):
    """
    First initializes
        - kafka listener - source of data
        - model - transformer of data
        - consumer - trigger of all calculations
    Then runs the while loop which
        - gets an s3 key with data as a message from kafka listener,
        - defines transforms of that data,
        - run transformation,
        - wait for another s3 key
    """
    ...
    # receive requests. @TODO abstractify.
...
def process_key(model, consumer, opts, s3_key: str) -> None:
    """
    Download s3 key, transform new content
